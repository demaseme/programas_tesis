...
\section{Cota inferior del anti-thickness geométrico de $K_n$}\label{sec:cota_inf}
  ...
  \begin{proof}
    ...
    corrección discutimos en la Sección~\ref{seccion_algoritmo_kthrackles}. Usando este conjunto
  ...
  Decimos que $\left\lceil\frac{n-1}{2}\right\rceil$ thrackles máximos son
  ...
  \begin{table}[t]
    ...
    \begin{tabular}{|c|c|c|c|}
      \hline
      $n$ & $\left\lceil\frac{n-1}{2}\right\rceil$ &     $\sum^n_{i=\left(n-\left\lceil\frac{n-1}{2}\right\rceil\right) + 1}i$ &
  ...
  \end{proof}
    ...
    intersecta en al menos una arista y que esto sucede para toda $n > 10$. Conjeturamos que este lema también se cumple para $n>10$.
...
\section{Cota superior del anti-thickness geométrico}\label{secc:descomposicion_thrackles_maximos}
  ...
  \begin{proof}
    ...
    colecciones de thrackles máximos que induzcan una descomposición cuya corrección es discutida
  ...
  \subsection{Número de cruce de thrackles máximos }\label{secc:cnthracklemax}
    ...
    \begin{proof}
      ...
      \binom{deg(u)}{2} = 0$ y el número de cruce es exactamente $\binom{m}{2}$. Sin embargo, debemos tener en cuenta que no necesariamente cada par de aristas se cruza sino que algunas pueden ser
...
\section{Anti-thickness de gráficas geométricas sin thrackles máximos}
  ...
  conjunto de $n$ puntos. El algoritmo es recursivo y usa \emph{backtracking}.
  ...
  el siguiente teorema. Además las descomposiciones están en la liga~\ref{link-decomposiciones-nomax} en formato \texttt{.csv} donde la primera columna representa el número de tipo de orden y la segunda columna
  representa la descomposición. Cada renglón de la segunda columna representa el conjunto de aristas del thrackle. Las aristas deben generadas como se describe en el Algoritmo~\ref{seccion_algoritmo_kthrackles}.
  \begin{theorem}
...
usando particiones guía}\label{secc:descomposiciones_particiones}
  ...
  siete, siete y seis. Para ninguna de estas particiones guía se encontraron los respectivos thrackles disjuntos. Para el resto de las particiones guía de $n=9$ escribimos un algoritmo para
