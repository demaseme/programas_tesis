...
\subsection{Algoritmo para encontrar thrackles con $k$
  ...
  $k$, donde $k$ es dada. Con este algoritmo buscamos resolver el siguiente problema:
...
inducen descomposiciones en thrackles de $K_n$}\label{secc:algo_descomposicion_thrackles_maximos}
  ...
  Este algoritmo necesita un paso de preparación para funcionar: Para cada tipo de orden, para
  $3\leq n \leq 10$, examinamos cuáles tienen al menos $n - \left\lfloor\sqrt{2n + \frac{1}{4}} -
  ...
  Después de la etapa de preparación, podemos empezar la búsqueda de las colecciones de thrackles
  máximos. Mostramos el pseudocódigo en el algoritmo~\ref{algo_descomposicion_thrackles_maximos}.
  Una vez que sabemos cuáles son los tipos de orden candidatos a tener una colección de thrackles
  máximos que induzcan una descomposición de $K_n$, buscamos, de manera exhaustiva, todas las
  combinaciones posibles de $n -  \left\lfloor\sqrt{2n + \frac{1}{4}} - \frac{1}{2}\right\rfloor$
  thrackles máximos. Para cada una de las combinaciones verificamos si esta cubre o no a las
  aristas de $K_n$. Si la combinación cubre, almacenamos las etiquetas en una lista de soluciones y
  examinamos la siguiente combinación. En caso contrario se descarta esa combinación y continuamos
  examinando.

  Las combinaciones fueron generadas usando el algoritmo descrito en~\cite{Knuth2011A}. Mostramos el psuedocódigo del algoritmo para generar las combinaciones en el algoritmo. Dicho
  algoritmo tiene una complejidad de $\displaystyle O\left(\binom{c}{t}\frac{t}{c-t+1}\right)$
  $t$ es el total de elementos y $c$ y es el tamaño de las combinaciones a generar.
  Con la información de la tabla~\ref{tabla:numero_operaciones_thrackles_maximos} podemos
  decir que $t$ es al menos $5$ y como buscamos descomposiciones cuyo tamaño sea igual al Anti-thickness de $K_n$, decimos que $c=n - \left\lfloor\sqrt{2n + \frac{1}{4}} - \frac{1}{2}\right\rfloor$. De esta manera y para este problema, el algoritmo de generación de combinaciones tiene una complejidad de \[O \left( \frac{5}{n - \left\lfloor\sqrt{2n + \frac{1}{4}} - \frac{1}{2}\right\rfloor - 4} \right) = O(n)\]
  % \begin{algorithm}[pb]
  %   \begin{algorithmic}[1]
...
\frac{1}{2}\right\rfloor$ posiciones y verificamos que solamente exista una
ocurrencia de $n$. De esta manera, el procedimiento de evaluación tiene una complejidad de $O(n)$ lo que causa que la
