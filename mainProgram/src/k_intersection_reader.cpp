...
void read_results(int set_size, int k, int t_size){
    ...
    int ot = 1;
    int otypes;
    int total_ots[8] = {1,2,3,16,135,3315,158817,14309547};
    otypes = total_ots[set_size-3];
    while(ot < otypes){
        usleep(2000000);
        //######################
        int rows = count_thrackles(set_size,t_size,ot);
        printf("Working with OT %d there are %d thrackles of size %d.\n",ot,rows,t_size);
        printf("There are %d rows\n",binomialCoeff(rows,k));
        if ( !rows || rows < k) {
            ot++;
            continue;
        }
        //The file contains, binom(rows,k) related to current order type
        //So n should be equal to binom(rows,k).
        //#####################
        int q = k;
        int counter =0;
        int c[q+3];
