...
void dibuja() {
	...
	for(i = 0; i < n; i++){
		for(int j = i+1; j < n; j++){
			int color_index = edge_matrix[edge_count][1]-1;
			glColor3f(r_colors[color_index],g_colors[color_index],b_colors[color_index]);
			glBegin(GL_LINE_STRIP);
			glVertex2i(puntos_otype[i].x, puntos_otype[i].y);
			glVertex2i(puntos_otype[j].x, puntos_otype[j].y);
			glEnd();
			sprintf(tag_char,"%d",edge_count);
			print( (puntos_otype[j].x + 3*puntos_otype[i].x)/4.0, 
			       (puntos_otype[j].y + 3*puntos_otype[i].y)/4.0,
			        tag_char );
			edge_count++;
		}
	}
...
void procesa_puntos(){
	...
	for(i = 0; i < n; i++){
		...
		printf("%f,%f\n",puntos_otype[i].x,puntos_otype[i].y);
	}
	...
	alto_otype = (ymax-ymin);
    	printf("xmin, ymin, ancho, alto %d %d %d %d\n",xmin,ymin,ancho_otype,alto_otype);
